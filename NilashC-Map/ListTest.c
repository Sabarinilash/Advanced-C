...
int main() {
  ...
  if (isInList(l,"Paris"))
    puts("Paris is in list");
  else
    puts("Paris is not in list");
  if (isInList(l,"Lyon"))
    puts(" Lyon is in list");
  else
    puts("Lyon is not in list");
  int c;
    FILE *file;
    file = fopen("H:\mapAdv-C\NilashC-Map\FRANCE.MAP", "r");
    if (file) {
